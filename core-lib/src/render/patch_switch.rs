...
//!
//! パッチの「形式」を知っているのは [`super::patch_state`]（`.fxp` と CLAP state）と
//! [`super::cartridge_patch`]（cartridge の 1 program）で、ここはそのどちらを使うかを
//! 決めて、いま何が載っているかを記録するだけ。
...
impl RealtimeRenderer {
    ...
    /// SysEx + Program Change になる（[`cartridge_patch`]）。
    ...
    fn plugin_instance_mut(&mut self) -> &mut PluginInstance<MidiRenderHost> {
